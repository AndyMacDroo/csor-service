...
class EventStreamAnalyserTest(unittest.TestCase):
    ...
    def test_event_stream_analyser_does_not_calls_handle_on_event_handler_with_empty_event(self):
        event_data_stream = ObjectRecognitionService(config=mock_config, video_stream={})
        event_data_stream.read_stream_for_event = MagicMock(return_value={})
        event_handler = VideoEventHandler(config=mock_config)
        event_handler.handle = MagicMock()
        event_stream_analyser = EventStreamAnalyser(config=mock_config, event_data_stream=event_data_stream, event_handler=event_handler)
